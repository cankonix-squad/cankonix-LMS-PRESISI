...
`AcademicSchedule`: id, classSubjectId, title, startAt, endAt, mode (FACE_TO_FACE/ONLINE/BLENDED), location/url nullable, status, metadata nullable.

## API / Application Contract
CRUD/list calendar by class/person/date range.

## Business Rules
endAt > startAt. Conflict detection minimal untuk exact resource/person overlap bila data educator tersedia; conflict menghasilkan validation/warning sesuai spec implementation, tidak silent.

## Acceptance Criteria
[ ] date-range API; [ ] invalid time rejected; [ ] basic conflict test; [ ] checks green.
