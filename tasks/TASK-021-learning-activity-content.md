...
`LearningActivityType`: id/code/name/status. `LearningActivity`: id, meetingId, typeId, sequence, title, instructions, required, availability dates, status. `LearningContent`: id, activityId, contentType, title, objectKey/externalUrl nullable, mimeType/size metadata nullable, version, status.

## API / Application Contract
CRUD activity; publish/unpublish; content metadata CRUD; signed/upload flow hanya melalui File service bila tersedia.

## Business Rules
Activity type data-driven. Binary wajib object storage. External link divalidasi. Content version/history dipertahankan untuk published use.

## Acceptance Criteria
[ ] multiple activity types; [ ] no binary DB; [ ] ordering; [ ] publish validation; [ ] checks green.
