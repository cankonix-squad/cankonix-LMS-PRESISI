...
## Objective
Membangun subject reusable dan curriculum versioned.
...
`Subject`: id, code unique, name, description, status. `Curriculum`: id, educationProgramId, version, name, effectiveFrom nullable, status, timestamps; unique program+version. `CurriculumSubject`: id, curriculumId, subjectId, sequence, creditHours/weight nullable, required boolean; unique curriculum+subject.

## API / Application Contract
CRUD subject; create/read curriculum version; add/remove/reorder curriculum subjects; list curriculum by program.

## Business Rules
Curriculum yang sudah dipakai batch tidak boleh dimutasi secara destruktif; buat version baru. Subject master reusable.

## Acceptance Criteria
[ ] version uniqueness; [ ] used curriculum protected; [ ] subject mapping unique; [ ] tests + checks green.
